...
os.environ["WANDB_API_KEY"] = '26f2e44ba8a86397ad3ae87fe4d5619c96cd94d3'
...
parser.add_argument('--corpus_type', type=str, default="baidu", help="NYT, WebNLG, NYT*, WebNLG*")
...
parser.add_argument('--multi_gpu', type=bool, default=True, help="ensure multi-gpu training")
...
parser.add_argument('--ensure_corres', type=bool, default=True, help="correspondence ablation")    # 对齐消融
# parser.add_argument('--ensure_rel', action='store_true', help="relation judgement ablation")
parser.add_argument('--ensure_rel', type=bool, default=True, help="relation judgement ablation")    # 关系判断消融
...
def train(model, data_iterator, optimizer, params, ex_params, val_loader):
...
def train_and_evaluate(model, params, ex_params, restore_file=None):
    ...
    logging.info(f"start load data:")
    ...
    logging.info("load data execution time: {} seconds".format(execution_time))
    ...
    print(f'Epoch={args.epoch_num}')
    for epoch in range(1, args.epoch_num + 1):
        # Run one epoch
        logging.info("Epoch {}/{}".format(epoch, args.epoch_num))

        # Train for one epoch on training set
        train(model, train_loader, optimizer, params, ex_params, val_loader)
...
if __name__ == '__main__': 
    ...
    ex_params = {
        ...
        'ensure_rel': args.ensure_rel,      # 关系判断消融
        'num_negs': args.num_negs,    # 笑消融关系判断时的负样本数
        'emb_fusion': args.emb_fusion
    }

    # wandb
    wandb.init(project="PRGC", name=params.run_name)
